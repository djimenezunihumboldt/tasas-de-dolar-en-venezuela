...
(async () => {
  try {
    ...
    const out = path.join(root, 'app', 'src', 'assets', 'kontigo-clean3.png');
    const size = 200;

    const base = sharp(input).resize(size, size, { fit: 'cover', position: 'center' }).png();
    const grayBuf = await base.clone().grayscale().toBuffer();

    // Threshold (no negate) so white corresponds to bright areas (the symbol white)
    ...
    console.error('Error processing kontigo3:', err);
    process.exit(1);
  }
})();
